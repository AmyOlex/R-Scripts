...
my.breaks<-c(seq(min(mtx[mtx!=min(mtx)]),-cutoff,length.out=granularity),seq(cutoff,max(mtx[mtx!=max(mtx)]),length.out=granularity)) #Breaks are going from second min(mtx) to -cutoff, then black then from cutoff to the second max(mtx)
...
for (d in dist.methods) {
  for (h in hclust.methods){
    ...
    # Without breaks, brewer colors. Best for enrichment only
    heatmap.2(as.matrix(mtx),trace="none",col=color,distfun=function(x){dist(x,method=d)}, hclustfun=function(x){hclust(x,method=h)}, density.info="none",cexCol=1,cexRow=0.8, notecex=1, main=paste("Dist : ",d,"; Hclust : ",h)) # cellnote=mtx.raw, notecol='darkgreen',
    # par(ask=T) # Wait for Return after each plot
  }
}
dev.off()
...
mtx=cor(mtx,method="k") # "pearson" (default), "kendall", or "spearman"
diag(mtx)<-0
